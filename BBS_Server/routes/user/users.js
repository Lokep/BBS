...
router.post('/resetPwd', (req, res, next) => {
  let resetInfo = req.body,
    sql = `SELECT * FROM ${USERINFO_TABLE} WHERE ${resetInfo.accountType} = ?`,
    params = [resetInfo.account];
  db.query(sql, params, (err, result) => {
    if (err) {
      console.log(`查询失败:${err}`);
      return false
    } else {
      let resetSql = `UPDATE ${USERINFO_TABLE} SET password = ? WHERE id = ?`,
        resetParams = [resetInfo.newPassword, result[0].id];
      db.query(resetSql, resetParams, (error, data) => {
        if (error) {
          console.log(`查询失败:${error}`);
          return false
        }
        if (data) {
          res.send({code: "200", message: "密码重置成功", data: data})
        } else {
          res.send({code: "404", message: "密码重置失败"})
        }
      })
    }
  });
